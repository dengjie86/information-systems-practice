...
> - PDF：[`assets/architecture.pdf`](./assets/architecture.pdf)
> - SVG：[`assets/architecture.svg`](./assets/architecture.svg)（矢量源文件）
...
> - PDF：[`assets/er-diagram.pdf`](./assets/er-diagram.pdf)
> - SVG：[`assets/er-diagram.svg`](./assets/er-diagram.svg)（矢量源文件）
...
中期阶段全部主要页面已完成并按 `页面截图清单.md` 的命名规则截图，统一放在 `docs/report/assets/` 下。下面按角色分组展示。
...
![学生首页](./assets/02-student-home.png)

**我的工单列表**：分页 + 状态筛选，列表行展示工单号、标题、状态徽章、提交时间。

![学生工单列表](./assets/03-student-list.png)

**提交报修**：分类下拉、标题、详细描述、上传图片、电话、优先级。

![报修提交页](./assets/04-student-create.png)

**工单详情**：基本信息 + 维修记录时间线（接单 / 处理 / 完成等条目按时间倒序）。

![工单详情](./assets/05-student-detail.png)

**确认 + 评价**：维修完成后由学生确认，5 星评分 + 评价内容。

![评价弹窗](./assets/06-student-evaluate.png)

#### 2.3 管理端（5 张）

**工单管理**：全工单列表 + 状态筛选 + 维修人员筛选 + 行内操作（审核 / 分派）。

![工单管理](./assets/07-admin-orders.png)

**审核工单**：弹窗内显示工单详情，"通过 / 驳回"按钮，驳回必须填原因。

![审核弹窗](./assets/08-admin-audit.png)

**分派维修员**：下拉选维修人员，可填分派备注。

![分派弹窗](./assets/09-admin-assign.png)

**故障分类管理**：分类表格 + 新增 / 编辑 / 启用停用 / 排序。

![分类管理](./assets/10-admin-categories.png)

**统计分析**：4 块内容——总览卡片、分类分布饼图、维修人员处理量柱状图、近 7 天趋势折线图。

![统计分析](./assets/11-admin-stats.png)

#### 2.4 维修端（3 张）

**我的派单**：按状态分组展示分给当前维修员的工单。

![维修员派单](./assets/12-worker-list.png)

**添加处理记录**：填写处理动作描述。

![添加处理记录](./assets/13-worker-record.png)

**完成维修**：最终说明 + 上传维修后照片，提交后工单进入"待确认"。

![完成维修](./assets/14-worker-finish.png)

...
中期阶段全部主要页面已完成并按统一命名规则截图，统一放在 `docs/report/assets/` 下。下面按角色分组展示。
...
**Newman 自动化**（覆盖健康检查 / 登录 / 分类 / 统计 / 越权 5 个目录，合计 22 个请求 / 10 个断言 / 0 失败）：
...
| 20 | 陈梓轩、尹添一 | 统计接口与图表 |
| 21 | 胡宇翔 | 测试用例、接口测试记录、Postman 集合与环境、Newman/PowerShell 端到端脚本、缺陷汇总 bug-list |
| 22 | 陈梓轩 | 演示数据补充（取消报修、近 7 天趋势、user.avatar、评价分布、上传图片入库） |
| 23 | 胡宇翔 | 中期检查材料：中期报告、系统架构说明、ER 图 |
...
- 相关文档（同仓库内）：
  ...
  - `docs/report/系统架构.md`、`docs/report/ER图.md`
- 其他补充材料：（结项时补）
